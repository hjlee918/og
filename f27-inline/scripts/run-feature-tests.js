...
//
//   node f27-inline/scripts/run-feature-tests.js
//
...
// `f27-inline/tests/feature-build.test.js`.
//
// The third suite is this batch's harness improvement: the error classifier the
// outgoing acceptance record required be tightened, driven deterministically.
//
...
status |= run(
  ...
  'replaced in f27-inline/tests/feature-build.test.js)',
  ['--test', `--test-skip-pattern=${SKIP}`].concat(PILOT_FILES));

status |= run('feature build identity, integrity, fixture rules and error classification',
