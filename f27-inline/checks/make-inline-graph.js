...
'use strict';
//
// Generates ONE fresh synthetic graph for the F27 INLINE-CONTEXT feature batch,
// from templates in this file, inside a uniquely named test-owned subfolder of
// the permitted graph-data root.
//
// It is the outgoing batch's fixture PLUS this slice's own reading pages. The
// outgoing shape is kept verbatim — including the exactly-ten sources of the
// anchor that the compact overview's row limit depends on — because the
// existing F27 behaviour it exercises is regression coverage for this batch.
//
...
//
//   INLINE CONTEXT (this slice)
//     * one sentence carrying a BARE reference and a LABELLED reference to two
//       DIFFERENT targets, with Korean, English and emoji around them, so two
//       panels can be proved independent and the labelled spelling is covered;
//     * one sentence writing the SAME target twice, so two occurrences of one
//       identity can be proved not to share state;
//     * a reference to a uuid no block carries, so the honest unavailable state
//       is reachable without deleting anything;
//     * a target with a PARENT, a CHILD and a link of its own, so the panel's
//       breadcrumb, ancestors, descendants and *Links in this block* sections
//       all have something true to say — and so the breadcrumb can be seen to
//       name the TARGET's page rather than the page being read;
//     * a reference to one side of a mutual pair, so a cycle inside the panel
//       is marked rather than followed;
//     * an EMBED surface whose embedded block contains an inline reference, so
//       an excluded surface can be seen to carry no control — and, written into
//       the SAME block, the mutual pair, so OG's own `max-depth-of-links`
//       ceiling can be observed where this slice adds nothing at all. That is
//       what separates "OG substitutes a mutual pair until its ceiling" from
//       "the new control caused it".
//
...
const UUID = {
  ...
  // ---- this slice's own shapes ----------------------------------------
  // The target an inline reference in running text points at. It has a
  // PARENT, a CHILD and a link of its own, so every section of the panel has
  // something true to say, and it lives on a page of its own so the panel's
  // breadcrumb can be seen to name the TARGET's page.
  inlineTarget: '65f27b00-0000-4000-8000-0000000000f1',

  // A second, different target in the SAME sentence, written with a label.
  inlineOther: '65f27b00-0000-4000-8000-0000000000f2',

  // The block an embed surface renders. It contains an inline reference, which
  // must carry NO control there.
  embedHost: '65f27b00-0000-4000-8000-0000000000f3',

  // A mutual pair of THIS slice's own, referenced from the reading page. It is
  // deliberately NOT the outgoing fixture's cycle: referencing that one would
  // add an inbound row to it and change what the preserved outgoing scenario
  // observes, which would make this batch's fixture edit the previous batch's
  // evidence.
  inlineCycleA: '65f27b00-0000-4000-8000-0000000000f4',
  inlineCycleB: '65f27b00-0000-4000-8000-0000000000f5',
};
...
`,
  ...
  // ---- this slice: ordinary reading content -----------------------------
  // None of these blocks references the anchor, so the compact overview's
  // ten-row rule above is untouched.
  //
  // Each sentence is one block, so a check can address one line of reading
  // text and know which references are in it.
  'Inline Reading.md': `- # Inline Reading
- 두 대상: bare ((${UUID.inlineTarget})) 그리고 🎯 [labelled other](((${UUID.inlineOther}))) in one sentence.
- The same target twice: ((${UUID.inlineTarget})) 한국어 사이 텍스트 and again ((${UUID.inlineTarget})).
- A reference to a block nobody wrote: ((${UUID.ghost})) — nothing may be created for it.
- One side of a mutual pair: ((${UUID.inlineCycleA})) whose own links point back.
- An embed surface, whose embedded block writes a reference of its own: {{embed ((${UUID.embedHost}))}}
`,

  'Inline Target Source.md': `- A PARENT block, which the panel's ancestors must show
	- 인라인 대상 블록 — the inline target, linking to ((${UUID.t1})) 🎯
	  id:: ${UUID.inlineTarget}
		- A CHILD of the inline target, which the panel's descendants reveal
- The other inline target, written with a label where it is referenced
  id:: ${UUID.inlineOther}
- A block that an EMBED renders, writing its own reference ((${UUID.t2})) and one side of the mutual pair ((${UUID.inlineCycleA}))
  id:: ${UUID.embedHost}
- Inline cycle side A, referencing ((${UUID.inlineCycleB}))
  id:: ${UUID.inlineCycleA}
- Inline cycle side B, referencing ((${UUID.inlineCycleA}))
  id:: ${UUID.inlineCycleB}
`,
};

function build(opts = {}) {
  const stamp = opts.stamp || new Date().toISOString().replace(/[:.]/g, '-');
  const name = `f27-inline-run-${stamp}`;
...
if (require.main === module) {
  const r = build();
  console.log(`[inline-graph] ${r.graph}`);
  console.log(`[inline-graph] ${r.pages} pages, 1 journal, 1 asset`);
}
