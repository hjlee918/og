...
test('a feature build is present in static/', () => {
  assert.ok(built,
    `no feature build present at ${manifestPath}; run f27-outgoing/scripts/build-feature.js first`);
});
...
test('this build does not claim to be the accepted pilot renderer', { skip: !built }, () => {
  ...
  assert.strictEqual(m.builtFrom.branch, 'feature/f27-outgoing-context');
...
test('the packaging configuration writes outside every accepted checkout',
  { skip: !built }, () => {
    ...
    for (const name of ['f27-slice-1', 'f27-pilot']) {
      assert.ok(!out.split(path.sep).includes(name), `${out} is inside ${name}`);
    }
    assert.ok(out.split(path.sep).includes('f27-outgoing-context'), out);
